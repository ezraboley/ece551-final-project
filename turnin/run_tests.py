...
def run_test(test_name):
    try:
        output = subprocess.check_output("{} {}".format(VSIM_EXE, test_name), shell=True)
        #subprocess.call("{} {}".format(VSIM_EXE, test_name), shell=True, stdout=subprocess.STDOUT)
        #print(output)
...
def main():
    ...
    for test in tests:
        print("\n\n\nRUNNING {}".format(test))
        run_test("work." + str(test) + " -do 'run -all' > results/" + str(test) + ".txt")
